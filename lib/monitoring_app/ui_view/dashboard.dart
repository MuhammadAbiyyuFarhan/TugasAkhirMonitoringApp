...
class _DashboardView extends State<DashboardView> {
  ...
  String beltSpeed2 = 'Loading...';
  String beltSpeedAVG = 'Loading...';
  String RPM1 = 'Loading...';
  String RPM2 = 'Loading...';
  ...
  void initState() {
    ...
    _databaseReference.onValue.listen((DatabaseEvent event) {
      if (event.snapshot.value != null) {
        //pake mounted supaya pas di dispose ga error
        if (mounted) {
          setState(() {
            // Use null-aware operators to safely access the values
            var data = event.snapshot.value as Map<Object?, Object?>?;
            beltSpeed1 = data?['beltSpeed1']?.toString() ?? 'Error';
            beltSpeed2 = data?['beltSpeed2']?.toString() ?? 'Error';
            beltSpeedAVG = data?['beltSpeedAVG']?.toString() ?? 'Error';
            RPM1 = data?['RPM1']?.toString() ?? 'Error';
            RPM2 = data?['RPM2']?.toString() ?? 'Error';
            RPMAVG = data?['RPMAVG']?.toString() ?? 'Error';
          });
        }
      } else {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget? child) {
        ...
        num beltSpeedAvgValue = double.tryParse(beltSpeedAVG) ?? 0;
                                  ...
                                  Padding(
                                    ...
                                    child: Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: <Widget>[
                                          Text(
                                            '${(beltSpeedAvgValue * widget.animation!.value).toDouble()}',
                                            textAlign: TextAlign.center,
                                            style: const TextStyle(
                                              fontFamily:
                                  ...
                                  Padding(
                                    ...
                                    child: CustomPaint(
                                      painter: CurvePainter(
                                          ...
                                          angle: 140 + (360 - 250) * (beltSpeedAvgValue -widget.animation!.value)),
